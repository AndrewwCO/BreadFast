        ...
        <View style={styles.content}>
          ...
          <View style={styles.timeRow}>
            {PICKUP_TIMES.map((slot) => (
              <TouchableOpacity
                key={slot.time}
                style={[styles.timeSlot, selectedTime === slot.time && styles.timeSlotActive]}
                onPress={() => setSelectedTime(slot.time)}
              >
                <Text style={[styles.timeText, selectedTime === slot.time && styles.timeTextActive]}>
                  {slot.time}
                </Text>
                <Text style={[styles.availableText, selectedTime === slot.time && styles.availableTextActive]}>
                  Available
                </Text>
              </TouchableOpacity>
            ))}
...
const styles = StyleSheet.create({
  ...
  heroSection: {
    backgroundColor: '#FFF4E8', height: 220, alignItems: 'center',
    justifyContent: 'center', borderBottomLeftRadius: 24, borderBottomRightRadius: 24,
  },
  heroEmoji: { fontSize: 100 },
  content: { paddingHorizontal: 24, paddingTop: 24, paddingBottom: 100 },
  productName: { fontSize: 26, fontWeight: '800', color: '#1A1A1A', letterSpacing: -0.5 },
  description: { fontSize: 14, color: '#888', lineHeight: 21, marginTop: 8 },
  price: { fontSize: 28, fontWeight: '800', color: '#F97316', marginTop: 12 },
  sectionLabel: {
    fontSize: 13, fontWeight: '600', color: '#444', marginTop: 24,
  ...
  qtyValue: { fontSize: 18, fontWeight: '700', color: '#1A1A1A', minWidth: 24, textAlign: 'center' },
  timeRow: { flexDirection: 'row', gap: 10 },
  timeSlot: {
    flex: 1, borderRadius: 14, borderWidth: 1.5, borderColor: '#E5E5E5',
    backgroundColor: '#fff', paddingVertical: 12, alignItems: 'center',
  },
  timeSlotActive: { backgroundColor: '#F97316', borderColor: '#F97316' },
  timeText: { fontSize: 15, fontWeight: '700', color: '#1A1A1A' },
  timeTextActive: { color: '#fff' },
  availableText: { fontSize: 11, color: '#888', marginTop: 2 },
  availableTextActive: { color: '#FFE0C2' },
  footer: {
    position: 'absolute', bottom: 0, left: 0, right: 0,
    padding: 20, paddingBottom: 32, backgroundColor: '#FFFBF5',
  },
  addToCartBtn: {
